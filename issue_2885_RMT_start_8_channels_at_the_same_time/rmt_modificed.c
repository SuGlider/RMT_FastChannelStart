...
// Translate uint8_t type of data into rmt format, write it to RMT memory, BUT DO NOT SENT IT OUT!
esp_err_t rmt_write_sample_no_start(rmt_channel_t channel, const uint8_t *src, size_t src_size)
{
  ESP_RETURN_ON_FALSE(RMT_IS_TX_CHANNEL(channel), ESP_ERR_INVALID_ARG, TAG, RMT_CHANNEL_ERROR_STR);
  ESP_RETURN_ON_FALSE(p_rmt_obj[channel], ESP_FAIL, TAG, RMT_DRIVER_ERROR_STR);
  ESP_RETURN_ON_FALSE(p_rmt_obj[channel]->sample_to_rmt, ESP_FAIL, TAG, RMT_TRANSLATOR_UNINIT_STR);
  ...
  //rmt_tx_start(channel, true);  // DO NOT START IT HERE ... do it latter in rmt_start_all_channels(), for instance.
  return ESP_OK;
}


// start all RMT channels available in a single call as fast as possible
esp_err_t rmt_start_all_channels()
{
  for (uint8_t channel = 0; channel < RMT_CHANNEL_MAX; channel++) {
    rmt_ll_clear_tx_end_interrupt(rmt_contex.hal.regs, channel);
    rmt_ll_enable_tx_end_interrupt(rmt_contex.hal.regs, channel, true);
    rmt_ll_tx_start(rmt_contex.hal.regs, channel);
  }
